...
export class SolicitacaoServices {
  ...
  public async SolicitacaoCreate(valid: SolicitacaoDto, id: string) {
    try {
      ...
      const getLastProtocolo = await SolicitacaoRepository.createQueryBuilder('Solicitacao')
        .orderBy('Solicitacao.log_criacao', 'DESC')
        .getOne()
      console.log(getLastProtocolo)
      var order = `${getLastProtocolo?.chamado.slice(8)}`
      var numbers = parseInt(order)
